...
if __name__ =="__main__":  
    title_episodes= pd.read_csv("./IMDb/title.episode.tsv",delimiter='\t',usecols=[0,1,2,3],na_values="\\N")
    title_episodes.dropna(inplace = True)
    ...
    final_titles_selected_from_ep = title_episodes[title_episodes['tconst'].isin(title_ratings_sorted_15000_rows['tconst'])]

    sql_path="./Data_Warehouse_Arhitecture/src/bronze/inserting/insert_title_episodes.sql"
    sql_query=read_sql(sql_path)
    for row in final_titles_selected_from_ep.itertuples(index=False):
        rand=list(row)
