...
if __name__ =="__main__":  
    title_directors= pd.read_csv("./IMDb/title.crew.tsv",delimiter='\t',usecols=[0,1],na_values="\\N")
    title_directors.fillna(value='Anonim')
    ...
    final_directors = title_directors[
        title_directors['tconst'].isin(title_ratings_sorted_15000_rows['tconst'])
        ]
    print(len(final_directors))
    sql_path="./Data_Warehouse_Arhitecture/src/bronze/inserting/insert_title_directors.sql"
    sql_query=read_sql(sql_path)
    for row in final_directors.itertuples(index=False):
        rand=list(row)
