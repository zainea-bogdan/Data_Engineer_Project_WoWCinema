...
if __name__ =="__main__":  
    title_basics= pd.read_csv("./IMDb/title.basics.tsv",delimiter='\t',usecols=[0,1,2,4,5,7,8])
    ...
    final_titles_selected_from_basics = title_basics[title_basics['tconst'].isin(title_ratings_sorted_15000_rows['tconst'])]

    sql_path="./Data_Warehouse_Arhitecture/src/bronze/inserting/insert_title_basics.sql"
    sql_query=read_sql(sql_path)
    for row in final_titles_selected_from_basics.itertuples(index=False):
        rand=list(row)
